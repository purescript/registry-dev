  ...
  outputs =
    ...
    let
      ...
      npmFileset = fileset.intersection (fileset.gitTracked ./.) (
        fileset.fileFilter (file: file.name == "package.json" || file.name == "package-lock.json") ./.
      );
      ...
      registryOverlay = final: prev: rec {
        ...
        registry =
          let
            ...
            #   $ nix flake update
            supportedCompilers = prev.lib.filterAttrs (
    ...
    flake-utils.lib.eachSystem supportedSystems (
      ...
      rec {
        ...
        checks = {
          ...
          purescript-format =
            pkgs.runCommand "purescript-format-check"
              {
                src = fileset.toSource {
                  root = ./.;
                  fileset = pureScriptFileset;
          ...
          integration =
            if pkgs.stdenv.isDarwin then
              ...
              let
                wiremockMappings = import ./nix/wiremock-mappings.nix;
                serverPort = 8080;
                githubPort = 9001;
                bucketPort = 9002;
                s3Port = 9003;
                  ...
                  registry = {
                    ...
                    config = {
                      ...
                      services.wiremock-github-api = {
                        enable = true;
                        port = githubPort;
                        mappings = wiremockMappings.github-api;
                      };
                      ...
                      services.wiremock-s3-api = {
                        ...
                        mappings = wiremockMappings.s3-api;
                      };

                      services.wiremock-bucket-api = {
                        enable = true;
                        port = bucketPort;
                        mappings = wiremockMappings.bucket-api;
                      };

                      services.wiremock-pursuit-api = {
                        enable = true;
                        port = pursuitPort;
                        mappings = wiremockMappings.pursuit-api;
                      };
                    };
                  };
                  client = {
