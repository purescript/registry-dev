...
import Registry.App.Env (GITHUB_EVENT, PACCHETTIBOTTI)
import Registry.App.Legacy.LenientVersion as LenientVersion
import Registry.App.Legacy.Manifest (LegacyCache, ManifestFetchError(..))
import Registry.App.Legacy.Manifest as Legacy.Manifest
import Registry.App.Legacy.Types (RawPackageName(..), RawVersion(..), rawPackageNameMapCodec)
import Registry.App.Monad.Cache (class MonadCache)
...
import Registry.App.Monad.Notify (class MonadNotify)
import Registry.App.Monad.Notify as Notify
import Registry.App.Monad.PackageSets (class MonadPackageSets, Change(..))
import Registry.App.Monad.PackageSets as PackageSets
import Registry.App.Monad.Pursuit (class MonadPursuit)
...
packageSetUpdate payload = Except.runExceptT do
  { issue, username } <- asks _.githubEvent
  ...
  latestPackageSet <- Except.ExceptT Registry.readLatestPackageSet
  ...
  manifestIndex <- Except.ExceptT Registry.readAllManifests
  ...
  unless (Map.isEmpty candidates.rejected) do
    Except.throwError $ String.joinWith "\n"
  ...
  if Map.isEmpty candidates.accepted then do
    Except.throwError "No packages in the suggested batch can be processed; all failed validation checks."
  else do
    let changeSet = candidates.accepted <#> maybe Remove Update
    Notify.notify "Attempting to build package set update."
    Except.ExceptT (PackageSets.upgradeAtomic latestPackageSet (fromMaybe prevCompiler payload.compiler) changeSet) >>= case _ of
      Nothing ->
        Except.throwError "The package set produced from this suggested update does not compile."
      Just packageSet -> do
        let commitMessage = PackageSets.commitMessage latestPackageSet changeSet (un PackageSet packageSet).version
        Except.ExceptT $ Registry.writePackageSet packageSet commitMessage
        Notify.notify "Built and released a new package set! Now mirroring to the package-sets repo..."
        Except.ExceptT $ Registry.mirrorPackageSet packageSet
        Notify.notify "Mirrored a new legacy package set."

-- | Run an authenticated package operation, ie. an unpublish or a transfer.
authenticated
  :: forall m r
   . MonadAsk { | PACCHETTIBOTTI + r } m
  => MonadRegistry m
  => MonadStorage m
  => MonadGitHub m
  => MonadNotify m
  ...
  => AuthenticatedData
  -> m (Either String Unit)
authenticated auth = Except.runExceptT case auth.payload of
  Unpublish payload -> do
    ...
    metadata <- Except.ExceptT (Registry.readMetadata payload.name) >>= case _ of
      Nothing -> do
        Log.error $ "No metadata found for package " <> PackageName.print payload.name
        Except.throwError $ "This package cannot be unpublished because it has not been published before (no metadata was found)."
      Just value -> pure value

    now <- nowUTC
    published <- case Operation.Validation.validateUnpublish now payload.version metadata of
      Left NotPublished ->
        Except.throwError $ "Cannot unpublish " <> formatted <> " because this version has not been published."
      Left AlreadyUnpublished ->
        Except.throwError $ "Cannot unpublish " <> formatted <> " because it has already been unpublished."
      Left InternalError -> do
        Log.error $ formatted <> " is listed as both published and unpublished, which should be impossible!"
        Except.throwError $ "Cannot unpublish " <> formatted <> " due to an internal error."
    ...
    case (un Metadata metadata).owners of
      ...
      Just owners -> do
        ...
        liftAff (Auth.verifyPayload pacchettiBotti owners auth) >>= case _ of
          Left error -> do
            ...
            Except.throwError $ "Could not unpublish " <> formatted <> " because we could not authenticate ownership of the package."
          Right _ -> do
            ...
            Except.ExceptT $ Storage.delete payload.name payload.version
            Except.ExceptT $ Registry.writeMetadata payload.name updated
            Except.ExceptT $ Registry.deleteManifest payload.name payload.version
  ...
  Transfer payload -> do
    ...
    metadata <- Except.ExceptT (Registry.readMetadata payload.name) >>= case _ of
      Nothing -> do
        Log.error $ "No metadata found for package " <> PackageName.print payload.name
        Except.throwError $ "This package cannot be transferred because it has not been published before (no metadata was found)."
      Just value -> pure value
      ...
      Just owners -> do
        ...
        liftAff (Auth.verifyPayload pacchettiBotti owners auth) >>= case _ of
          Left error -> do
            ...
            Except.throwError $ "Could not transfer your package because we could not authenticate your ownership."
          Right _ -> do
            ...
            Except.ExceptT $ Registry.mirrorLegacyRegistry payload.name payload.newLocation
...
publish
  ...
  -> PublishData
  -> m (Either String Unit)
publish source payload = Except.runExceptT do
  ...
  Metadata existingMetadata <- Except.ExceptT (Registry.readMetadata payload.name) >>= case _ of
    Nothing -> case payload.location of
      Nothing -> do
        ...
        Except.throwError $ "Cannot register " <> printedName <> " because the payload did not include a 'location' field."
      Just location ->
  ...
  hadPursJson <- liftEffect $ FS.Sync.exists packagePursJson
  unless hadPursJson do
    ...
    address <- case existingMetadata.location of
      Git _ -> Except.throwError "Legacy packages can only come from GitHub."
      GitHub { subdir: Just subdir } -> Except.throwError $ "Legacy packages cannot use the 'subdir' key, but this package specifies a " <> subdir <> " subdir."
      GitHub { owner, repo } -> pure { owner, repo }

    version <- case LenientVersion.parse payload.ref of
      Left _ -> Except.throwError $ "The provided ref " <> payload.ref <> " is not a version of the form X.Y.Z or vX.Y.Z, so it cannot be used."
    ...
    Legacy.Manifest.fetchLegacyManifest payload.name address (RawVersion payload.ref) >>= case _ of
      ...
      Right legacyManifest -> do
        ...
        liftAff $ writeJsonFile Manifest.codec packagePursJson manifest

  Log.debug "A valid purs.json is available in the package source."

  Manifest manifest <- liftAff (Aff.attempt (FS.Aff.readTextFile UTF8 packagePursJson)) >>= case _ of
    Left error -> do
      Log.error $ "Could not read purs.json from path " <> packagePursJson <> ": " <> Aff.message error
      Except.throwError $ "Could not find a purs.json file in the package source."
    Right string -> liftAff (jsonToDhallManifest string) >>= case _ of
      Left error -> do
        Log.error $ "Manifest does not typecheck: " <> error
        Except.throwError $ "Found a valid purs.json file in the package source, but it does not typecheck."
      Right _ -> case parseJson Manifest.codec string of
        Left err -> do
          Log.error $ "Failed to parse manifest: " <> CA.printJsonDecodeError err
          Except.throwError $ "Found a purs.json file in the package source, but it could not be decoded."
        Right manifest -> do
  ...
  when (Operation.Validation.isMetadataPackage (Manifest manifest)) do
    Except.throwError "The `metadata` package cannot be uploaded to the registry because it is a protected package."
  ...
  unregisteredRef <- liftEffect $ Ref.new Map.empty
  forWithIndex_ verifiedResolutions \name version -> do
    Except.ExceptT (Registry.readMetadata name) >>= case _ of
      Nothing -> liftEffect $ Ref.modify_ (Map.insert name version) unregisteredRef
      Just (Metadata { published }) -> case Map.lookup version published of
        Nothing -> liftEffect $ Ref.modify_ (Map.insert name version) unregisteredRef
        Just _ -> pure unit
  unregistered <- liftEffect $ Ref.read unregisteredRef
  ...
  for_ (Operation.Validation.validateTarballSize bytes) case _ of
    Operation.Validation.ExceedsMaximum maxPackageBytes ->
      Except.throwError $ "Package tarball is " <> show bytes <> " bytes, which exceeds the maximum size of " <> show maxPackageBytes <> " bytes."
  ...
  when (bytes < minBytes) do
    Except.throwError $ "Package tarball is only " <> Number.Format.toString bytes <> " bytes, which indicates the source was not correctly packaged."
  ...
  compilationResult <- Except.runExceptT $ compilePackage
  ...
  Except.ExceptT $ Storage.upload manifest.name manifest.version tarballPath
  ...
  Except.ExceptT $ Registry.writeMetadata manifest.name (Metadata newMetadata)
  ...
  Except.ExceptT $ Registry.writeManifest (Manifest manifest)

  when (source == Current) $ case compilationResult of
    Left error -> do
      Log.error $ "Compilation failed, cannot upload to pursuit: " <> error
      Except.throwError "Cannot publish to Pursuit because this package failed to compile."
    Right dependenciesDir -> do
      ...
      Notify.notify "Successfully published docs to pursuit!"

  Except.ExceptT $ Registry.mirrorLegacyRegistry payload.name newMetadata.location
  Notify.notify "Mirrored location change to the legacy registry."
...
verifyResolutions
  ...
  -> ExceptT String m (Map PackageName Version)
  ...
  case resolutions of
    Nothing -> case Operation.Validation.validateDependenciesSolve manifest manifestIndex of
      Left errors -> do
        ...
        Except.throwError printedError
...
compilePackage { packageSourceDir, compiler, resolutions } = do
  ...
  Except.except $ handleCompiler dependenciesDir compilerOutput
  where
  ...
  installPackage dir name version = do
    ...
    Except.ExceptT $ Storage.download name version filepath
...
publishToPursuit { packageSourceDir, dependenciesDir, compiler, resolutions } = do
  ...
  publishJson <- case compilerOutput of
    ...
    Right publishResult -> do
      ...
      case Array.last lines of
        Nothing -> Except.throwError "Publishing failed because of an unexpected compiler error. cc @purescript/packaging"
        Just jsonString -> case Argonaut.Parser.jsonParser jsonString of
          Left err -> Except.throwError $ String.joinWith "\n"
  ...
  case result of
    Left error ->
      Except.throwError $ "Could not publish your package to Pursuit because an error was encountered (cc: @purescript/packaging): " <> error
    Right _ ->
...
fetchPackageSource
  ...
  -> ExceptT String m { packageDirectory :: FilePath, publishedTime :: DateTime }
fetchPackageSource { tmpDir, ref, location } = case location of
  Git _ -> do
    ...
    Except.throwError "Packages are only allowed to come from GitHub for now. See #15"

  GitHub { owner, repo, subdir } -> do
    -- TODO: Support subdir. In the meantime, we verify subdir is not present. (See #16)
    when (isJust subdir) $ Except.throwError "`subdir` is not supported for now. See #16"
    ...
    case pursPublishMethod of
      ...
      LegacyPursPublish -> do
        ...
        let
          ...
          clonePackageAtTag :: Aff (Either String Unit)
          clonePackageAtTag = do
            ...
            withBackoff' (Except.runExceptT (Git.gitCLI args Nothing)) >>= case _ of
              Nothing -> pure $ Left $ "timed out attempting to clone git tag: " <> url <> " " <> ref
              Just (Left err) -> pure $ Left err
              Just (Right _) -> pure $ Right unit

        liftAff clonePackageAtTag >>= case _ of
          Left error -> do
            Log.error $ "Failed to clone git tag: " <> error
            Except.throwError $ "Failed to clone repository " <> owner <> "/" <> repo <> " at ref " <> ref
        ...
        let
          getRefTime = do
            timestamp <- Git.gitCLI [ "log", "-1", "--date=iso8601-strict", "--format=%cd", ref ] (Just repoDir)
        ...
        publishedTime <- Except.runExceptT getRefTime >>= case _ of
          Left error -> do
            Log.error $ "Failed to get published time: " <> error
            Except.throwError $ "Cloned repository " <> owner <> "/" <> repo <> " at ref " <> ref <> ", but could not read the published time from the ref."
      ...
      PursPublish -> do
        ...
        commitDate <- do
          ...
          commit <- GitHub.getRefCommit { owner, repo } (RawVersion ref) >>= case _ of
            Left githubError -> do
              ...
              Except.throwError $ "Failed to fetch commit data associated with " <> destination <> " at ref " <> ref
          ...
          GitHub.getCommitDate { owner, repo } commit >>= case _ of
            Left githubError -> do
              ...
              Except.throwError $ "Unable to get published time for commit " <> commit <> " associated with the given ref " <> ref
            Right a -> pure a
        ...
        case response of
          Nothing -> Except.throwError $ "Could not download " <> archiveUrl
          Just (Left error) -> do
            Log.error $ "Failed to download " <> archiveUrl <> " because of an HTTP error: " <> Affjax.Node.printError error
            Except.throwError $ "Could not download " <> archiveUrl
          ...
          Just (Right { body }) -> do
            ...
            buffer <- liftEffect $ Buffer.fromArrayBuffer body
            liftAff (Aff.attempt (FS.Aff.writeFile absoluteTarballPath buffer)) >>= case _ of
              Left error -> do
                Log.error $ "Downloaded " <> archiveUrl <> " but failed to write it to the file at path " <> absoluteTarballPath <> ":\n" <> Aff.message error
                Except.throwError $ "Could not download " <> archiveUrl <> " due to an internal error."
              Right _ ->
        ...
        Foreign.Tar.getToplevelDir absoluteTarballPath >>= case _ of
          Nothing ->
            Except.throwError "Downloaded tarball from GitHub has no top-level directory."
          Just dir -> do
...
getPacchettiBotti :: forall m r. MonadAsk { | PACCHETTIBOTTI + r } m => m Owner
getPacchettiBotti = do
  { publicKey } <- asks _.pacchettiBotti
  pure $ Owner
