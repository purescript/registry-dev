...
publish maybeLegacyIndex payload = do
  ...
  case Operation.Validation.isNotPublished (Manifest receivedManifest) (Metadata metadata) of
    ...
    Nothing -> do
      ...
      let newPublishedVersion = { hash, ref: payload.ref, compilers: Just $ NonEmptyArray.singleton payload.compiler, publishedTime, bytes }
      ...
      let compilersMetadata = newMetadata { published = Map.update (Just <<< (_ { compilers = Just (NonEmptySet.toUnfoldable1 validCompilers) })) (un Manifest manifest).version newMetadata.published }
